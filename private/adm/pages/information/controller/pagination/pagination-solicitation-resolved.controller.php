...
if(!empty($page)){
    ...
    $dados = "<ul class='pagination'>";
        ...
        //botão para voltar
        if ($prev_page != 0) { 
            $dados .= "<li class='page-item'><a class='page-link pagination-last normal-14-medium-p' href='#' onclick='listSolicitationResolved($prev_page)' tabindex='-1' aria-disabled='true'>Anterior</a></li>";
            } else { 
        ...
        for ($i = 0; $i < $page_qnt ; $i++) { 
            $j = $i + 1;
            $dados .= "<li class='page-item'><a class='page-link pagination-page normal-14-medium-p' href='#' onclick='listSolicitationResolved($j)'> $j </a></li>";
        }
            
        //botão para avançar
        if ($next_page <= $page_qnt) {
            $dados .= "<li class='page-item'><a class='page-link pagination-next normal-14-medium-p' href='#' onclick='listSolicitationResolved($next_page)' tabindex='-1' aria-disabled='true'>Próximo</a></li>";
            } else { 
