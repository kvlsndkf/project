...
if(empty($search)){
    ...
    for ($i = 0; $i < count($listNewSolicitation); $i++) {
        ...
        $dados .= $styleButton = $row->status == "Nova" ? '' : 'd-none'; 
        $dados .= "<form id='register' action='./analyzing-solicitation.controller.php?solicitationNewID= $row->id ' class=' $styleButton' method='POST'>";
            $dados .= "<button type='submit' name='analyzeRequest'>Mover para análise e adicionar informação</button>";
        $dados .="</form>";
