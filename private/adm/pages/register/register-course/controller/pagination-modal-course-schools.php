...
if(isset($_GET['idCourse'])){
    $id = $_GET['idCourse'];
        ...
        $result_register = $connection->prepare("SELECT COUNT(*) AS 'qtd' FROM schoolsHasCourses sc
        ...
        $dados = "<input type='hidden' id='data-id' value='$id'>";
            ...
            //botão para voltar
            if ($prev_page != 0) { 
                $dados .= "<li class='page-item'><a class='page-link-modal pagination-last normal-14-medium-p' href='#' onclick='enterDataSchools($id,$prev_page)' tabindex='-1' aria-disabled='true'>Anterior</a></li>";
                } else { 
            ...
            for ($i = 0; $i < $page_qnt ; $i++) { 
                $j = $i + 1;
                $dados .= "<li class='page-item'><a class='page-link-modal pagination-page normal-14-medium-p' href='#' onclick='enterDataSchools($id,$j)'> $j </a></li>";
            }
                
            //botão para avançar
            if ($next_page <= $page_qnt) {
                $dados .= "<li class='page-item'><a class='page-link-modal pagination-next normal-14-medium-p' href='#' onclick='enterDataSchools($id,$next_page)' tabindex='-1' aria-disabled='true'>Próximo</a></li>";
                } else { 
