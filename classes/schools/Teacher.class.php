...
class Teacher
{
    ...
    public function listTeacher(string $search = ''): array | false
    {
        ...
        try {
            ...
            $stmt = $connection->prepare("SELECT id, name, photo FROM teachers ORDER BY name");
            $stmt->execute();
    ...
    private function searchTeacher(string $search): array | false
    {
        ...
        try {
            $stmt = $connection->prepare("SELECT * FROM teachers WHERE name LIKE '%$search%' ORDER BY name");
